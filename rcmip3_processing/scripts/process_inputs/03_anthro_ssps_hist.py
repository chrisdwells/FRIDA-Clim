...
for scen in ssps:
    ...
    df_ssp.to_csv(f'{outdir}/anthro_inputs_{scen}.csv')
...
f = FAIR()
f.define_time(start_year, hist_end_year, 1)
...
df_hist['Year'] = np.arange(start_year, hist_end_year+1)
df_hist = df_hist.set_index('Year')

# hfcs        
hfc134a_eq = np.zeros(n_years_hist)
for gas in f_gases:
    hfc134a_eq = hfc134a_eq + f.concentration[:,f.scenarios.index(scen),
              0,f.species.index(gas)] * radeff[gas] / radeff['HFC-134a']
...
new_ems = np.zeros(n_years_hist)
for i in range(1, n_years_hist):
    new_ems[i] = (hfc134a_eq_minus_baseline[i] - hfc134a_eq_minus_baseline[i-1
                       ]*decay_factor)/concentration_per_emission
...
df_hist.to_csv(f'{outdir}/anthro_inputs_hist.csv')
