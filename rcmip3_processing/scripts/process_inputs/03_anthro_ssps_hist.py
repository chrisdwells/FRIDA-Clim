...
for scen in ssps:
    ...
    # make with CO2 conc for esm-ssp (except the ssp370 variants which are only allGHG)
    df_ssp_CO2_conc = copy.deepcopy(df_ssp)
    
    df_ssp_CO2_conc = df_ssp_CO2_conc.drop(columns=['Emissions.CO2 Emissions from Fossil use'])
    df_co2 = conc_in[
        (conc_in['Scenario'] == scen) &
        (conc_in['Region'] == 'World') &
        (conc_in['Variable'] == 'Atmospheric Concentrations|CO2')
    ]
    df_ssp_CO2_conc['CO2 Forcing.Atmos CO2 exogenous'] = process_df(df_co2, year_cols)
    
    df_ssp_CO2_conc.to_csv(f'{outdir}/esm-{scen}.csv')

    
    # and make with CO2, CH4, N2O conc for ssp ie conc-driven
    df_ssp_ghg_conc = copy.deepcopy(df_ssp)
    ...
    df_ssp_ghg_conc['N2O Forcing.Atmos N2O exogenous'] = process_df(df_n2o, year_cols)
    ...
    df_ssp_ghg_conc['CH4 Forcing.Atmos CH4 exogenous'] = process_df(df_ch4, year_cols)
...
for scen in ['historical', 'historical-cmip6', 'hist-GHG', 'hist-aer', 'hist-CO2']:
    ...
    # and process co2 conc version
    df_co2_conc = copy.deepcopy(df_hist)
    df_co2_conc['CO2 Forcing.Atmos CO2 exogenous'] = process_df(df_co2, year_cols)

    df_co2_conc.to_csv(f'{outdir}/esm-{scen_name}.csv')
