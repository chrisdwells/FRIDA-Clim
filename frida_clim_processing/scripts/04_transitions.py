...
for scen in ssps:
    ...
    assert df_transitions_hist.index[-1] + 1 == df_transitions_scen.index[0]
    ...
    df_transitions.to_csv(f'../data/processed_for_frida/intermediates/frida_clim_land_transitions_hist_{scen}.csv')
    
    if scen == 'ssp245':
        df_transitions_hist_only = df_transitions.loc[start_year:hist_end_year]
        
        df_transitions_hist_only.to_csv('../data/processed_for_frida/priors_inputs/frida_clim_land_transitions_hist.csv')
