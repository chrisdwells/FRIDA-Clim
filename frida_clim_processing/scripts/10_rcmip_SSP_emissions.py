...
# makes the SSP emissions timeseries for doing the SSP projections, and for 
# the RCMIP calbibration 
...
# forcings to take from frida_iam inputs
common_forcings = [
    'Minor GHGs Forcing.Montreal Gases Effective Radiative Forcing', 
    'Ozone Forcing.Montreal gases equivalent effective stratospheric chlorine', 
...
for scen in ssps:
    ...
    df_ssp.to_csv(f'../data/processed_for_frida/intermediates/emissions_{scen}.csv')
    
    if scen == 'ssp245':
        
        df_hist = df_ssp[df_ssp.index <= hist_end_year]
        
        df_frida_iam = pd.read_csv('../data/processed_for_frida/priors_inputs/forcings_frida_iam.csv')
        df_frida_iam = df_frida_iam.set_index('Year')
        df_common = df_frida_iam[common_forcings]
        
        df_hist_full = pd.concat((df_hist, df_common), axis=1)
        
        df_hist_full.to_csv('../data/processed_for_frida/priors_inputs/forcings_rcmip.csv')
...
df_baseline.to_csv('../data/processed_for_frida/priors_inputs/emissions_baseline_rcmip.csv', index=False)
...
for calib in ['frida_iam', 'rcmip']:
    os.makedirs(f'../../calibration/{calib}/data/ssps_output/', exist_ok=True)
    for scen in ssps:
        for output in output_list:
            needed_csvs.append(f'../../calibration/{calib}/data/ssps_output/{scen}_{output}.csv')
