...
temp_hist = df_temp.values

axs.plot(time, np.median(temp_hist, axis=1), color="#000000", alpha=0.2)
axs.fill_between(time, np.percentile(temp_hist, 84, axis=1), 
              np.percentile(temp_hist, 16, axis=1), color="#000000", alpha=0.2)
...
#%%
df_air_sea_co2 = pd.read_csv("../data/priors_output/priors_ocean_CO2_flux.csv")

time2 = df_air_sea_co2['Year']

df_air_sea_co2 = df_air_sea_co2.drop(columns='Year')

air_sea_co2_hist = np.nanmask(df_air_sea_co2.values)

axs[1].plot(time2, np.median(air_sea_co2_hist, axis=1), color="#000000", alpha=0.2)
axs[1].fill_between(time2, np.percentile(air_sea_co2_hist, 84, axis=1), 
              np.percentile(air_sea_co2_hist, 16, axis=1), color="#000000", alpha=0.2)

axs[1].set_title('Air-sea CO2')
axs[1].set_ylabel('GtC/yr')
