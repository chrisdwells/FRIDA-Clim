...
column_list = variable_stock_list_frida + ['Energy Balance Model.Surface Temperature 1850 to 1900 offset relative to 1750[1]']
...
for n_i in np.arange(output_ensemble_size):
    ...
    for stock in variable_stock_list:
         row.append(df_1980_inits[f'="Run {n_i+1}: {stock}[1]"'].values[0])
...
df_combined = pd.concat([df_posterior_params, df_inits_out], axis=1)
