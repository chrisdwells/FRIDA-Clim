...
for var in varlist:
    ...
    for s_i, scen in enumerate(ssps.keys()):
        ...
        ssp_in = pd.read_csv(f'../{calibration}/data/ssps_output/{scen}_output.csv')
    ...
    plt.tight_layout()
    plt.savefig(
        f"../{calibration}/plots/ssps/{varname}.png"
    )
...
for s_i, scen in enumerate(ssps.keys()):
    ...
    df_scen = pd.read_csv(f'../{calibration}/data/ssps_output/{scen}_output.csv')
    legend = False
    if scen == 'ssp119':
        legend=True
...
plt.tight_layout()
plt.savefig(
    f"../{calibration}/plots/ssps/carbon_response.png"
)
...
fair_vars = {
    "Energy Balance Model.Land & Ocean Surface Temperature[1]":'temperature',
    "CO2 Forcing.Atmospheric CO2 Concentration[1]":'co2_concentration',
    "Energy Balance Model.ocean heat content change[1]":'ocean_heat_content_change',
    }

vars_plot = {
    "Energy Balance Model.Land & Ocean Surface Temperature[1]":['GMST (K)',-1, 13],
...
for var in vars_plot.keys():
    ...
    for scen in scens_plot:
        ...
        if var in fair_vars.keys():
            
            df_fair = pd.read_csv(f'../data/external/fair_input/{fair_vars[var]}_{scen}.csv')
            
            offsets = {}
            for perc in ['0.05', '0.17', '0.5', '0.83', '0.95']:
                offsets[perc] = 0
                if var in offset_vars.keys():
                    offsets[perc] = df_fair.loc[(df_fair['timebounds'] >= 1850) 
                                                & (df_fair['timebounds'] <= 1900),
                                        perc].mean()

            ax[c].plot(df_fair['timebounds'], df_fair['0.5'] - offsets['0.5'], color='grey')
            ax[c].fill_between(time, df_fair['0.05'] - offsets['0.05'], df_fair['0.95'] - offsets['0.95'], 
                             color='grey', linewidth=0, alpha=0.2, hatch='//')
            # ax[c].fill_between(time, df_fair['0.17'] - offsets['0.17'], df_fair['0.83'] - offsets['0.83'], 
            #                  color=color, linewidth=0, alpha=0.2, hatch='//')
    ...
    for scen in scens_plot:
        ...
        ax[c].set_ylabel('')
        if c % 4 == 0:
            ax[c].set_ylabel('Carbon fluxes (GtC/yr)')
...
# handles.append(mpatches.Patch(facecolor=leg_color, edgecolor=leg_color, linewidth=0, alpha=0.4, label='17-83 percentile'))
...
ax[0].legend(handles=handles, ncol=1, fontsize=10)

plt.suptitle(f'GMST, Carbon response, {output_ensemble_size} members')
plt.tight_layout()
plt.savefig(
    f"../{calibration}/plots/ssps/fig6_paper.png"
)

#%%

vars_plot = {
    "CO2 Forcing.Atmospheric CO2 Concentration[1]":['ppm', 200, 2200],
    "Ocean.Air sea co2 flux[1]":['Air-sea flux (GtC/year)', -2, 7.5],
    "Terrestrial Carbon Balance.Terrestrial carbon balance[1]":['TCB (GtC/year)', -3.5, 7],
    }

fig, ax = plt.subplots(len(vars_plot), 4, figsize=(14, 4*len(vars_plot)))
ax = ax.ravel()
...
ax[0].legend(handles=handles, ncol=1, fontsize=8)

plt.suptitle(f'CO$_2$ concentration, Air-sea CO$_2$ flux, Terrestrial Carbon Balance, {output_ensemble_size} members')
plt.tight_layout()
plt.savefig(
    f"../{calibration}/plots/ssps/figS5_paper.png"
)
