...
print("Running SSP scenarios RCMIP...")
...
df_volcanic = pd.read_csv(
    "data/input/volcanic_ERF_1750-2101_timebounds.csv",
...
for scenario in scenarios:
    f.concentration.sel(specie="CO2").sel(scenario=scenario).quantile((0.05,0.17,0.50,0.83,0.95), dim='config').to_pandas().T.to_csv(
        f'../../data/external/fair_input/RCMIP/co2_concentration_{scenario}.csv')
    f.temperature.sel(layer=0, scenario=scenario).quantile((0.05,0.17,0.50,0.83,0.95), dim='config').to_pandas().T.to_csv(
        f'../../data/external/fair_input/RCMIP/temperature_{scenario}.csv')
    f.ocean_heat_content_change.sel(scenario=scenario).quantile((0.05,0.17,0.50,0.83,0.95), dim='config').to_pandas().T.to_csv(
        f'../../data/external/fair_input/RCMIP/ocean_heat_content_change_{scenario}.csv')
